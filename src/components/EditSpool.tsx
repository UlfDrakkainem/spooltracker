...
export default function EditSpool() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [brands, setBrands] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [showScanner, setShowScanner] = useState(false);
  const [originalCreatedAt, setOriginalCreatedAt] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    const fetchSpool = async () => {
      try {
        const docRef = doc(db, 'spools', id);
        ...
        if (docSnap.exists() && docSnap.data().ownerId === auth.currentUser?.uid) {
          ...
          setIsMultiColor(data.isMultiColor || false);
          setColorHexes(data.colorHexes || [data.colorHex || '#000000', '#ffffff']);
          setNozzleTemp(data.nozzleTemp || '');
          setBedTemp(data.bedTemp || '');
          setBarcode(data.barcode || '');
          setInitialWeight(data.initialWeight);
          setRemainingWeight(data.remainingWeight);
          setEmptySpoolWeight(data.emptySpoolWeight);
          setSpoolType(data.spoolType);
          setOriginalCreatedAt(data.createdAt);
        } else {
          navigate('/');
        }
      } catch (error) {
        handleFirestoreError(error, OperationType.GET, `spools/${id}`);
      }
    };
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      fetchSpool().then(() => setLoading(false));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!auth.currentUser || !brandId || !id) return;

    const brand = brands.find(b => b.id === brandId);
    if (!brand) return;

    try {
      await updateDoc(doc(db, 'spools', id), {
        brandId,
      ...
      handleFirestoreError(error, OperationType.UPDATE, `spools/${id}`);
    }
  };

  if (loading) return <div className="p-8 text-center text-zinc-500 dark:text-zinc-400">Ładowanie danych szpuli...</div>;

  return (
    <div className="max-w-xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        <button onClick={() => navigate('/')} className="p-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">
          <ArrowLeft className="w-5 h-5 text-zinc-600 dark:text-zinc-400" />
        </button>
        <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">Edytuj Szpulę</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-zinc-900 p-6 rounded-2xl shadow-sm border border-zinc-200 dark:border-zinc-800 space-y-4 transition-colors">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select value={brandId} onChange={e => setBrandId(e.target.value)} className="w-full border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white rounded-xl px-3 py-2 focus:ring-2 focus:ring-blue-500 outline-none transition-colors">
              {brands.map(b => (
        ...
        <div className="grid grid-cols-3 gap-4 pt-4 border-t border-zinc-100 dark:border-zinc-800 transition-colors">
          <div>
            <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Waga Początkowa (g)</label>
          ...
          <div>
            <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Pozostało (g)</label>
            <input type="number" value={remainingWeight} onChange={e => setRemainingWeight(Number(e.target.value))} required min="0" className="w-full border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white rounded-xl px-3 py-2 focus:ring-2 focus:ring-blue-500 outline-none transition-colors" />
          </div>
          <div>
            <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Pusta Szpula (g)</label>
